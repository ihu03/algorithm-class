...
class RingBuffer:
    ...
    def read(self):
        # 링버퍼에서 가장 오래된 항목을 읽고 제거
        return self.q.dequeue()
    
    def enqueue2(self, item):   
        # 링버퍼에서 포화상태일 때 새 데이터를 삽입 연산
        q = self.q
        q.rear = (q.rear+1) % q.N 
        if q.rear == q.front: # full 이면(원형 큐인 경우 오버플로우 발생)
            q.front = (q.front +1) % q.N # overwirte 발생 (가장 오래된 데이터 drop)
...
def quiz_3():
    ...
    #1. capacity 8인 링버퍼 생성
    rb =RingBuffer(capacity=8)
    rb.display("초기 링버퍼 상태")

    #2. 공백상태의 링버퍼에 1,2,3,...,20 순서대로 삽입
    for i in range(1, 21):
        rb.enqueue2(i)
        print(i,end=" ")
        rb.display("삽입 후: ")
